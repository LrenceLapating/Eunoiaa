...
router.post('/generate', verifyCounselorSession, async (req, res) => {
  try {
    ...
    // Get student data with latest assessment
    const { data: studentAssessment, error: fetchError } = await supabase
    ...
    if (fetchError || !studentAssessment) {
      console.error('Error fetching student assessment:', fetchError);
...
router.post('/auto-generate-all', verifyCounselorSession, async (req, res) => {
  try {
    ...
    // Query from assessments_42items table which has the actual assessment data
    const { data: studentsNeedingInterventions, error: fetchError } = await supabase
    ...
    studentsNeedingInterventions.forEach(assessment => {
