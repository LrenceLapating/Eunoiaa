...
class AIService {
  ...
  createInterventionPrompt(studentData, riskLevel) {
    ...
    // ULTRA-SHORT PROMPT - MAXIMUM CLARITY
...
Your overall score is ${Math.round(overallScore || 0)} out of ${overallMaxScore}. [Write a comprehensive 4-5 sentence analysis of their overall well-being. CRITICAL INSTRUCTIONS: 
1. You MUST explicitly mention and celebrate the healthy dimensions by name if any exist in the HEALTHY DIMENSIONS list above. Say something like "You have healthy levels in [list the healthy dimensions]" or "Your strengths include [healthy dimensions]".
2. You MUST explicitly mention the moderate dimensions by name if any exist in the MODERATE DIMENSIONS list above. Say something like "You have moderate levels in [list the moderate dimensions]" or "Areas showing moderate progress include [moderate dimensions]" - make this sound encouraging, like they are on a good path between maintaining and improving.
3. You MUST explicitly mention the at-risk dimensions by name if any exist in the AT RISK DIMENSIONS list above. Say something like "The areas that need attention are [list at-risk dimensions]".
4. Do NOT mention any dimension status unless it appears in the respective lists above.
5. Make it detailed and personalized based on their scores.]

Dimension Scores & Targeted Interventions:

Autonomy (${Math.round(subscales?.autonomy || 0)}/${dimensionMaxScore}): Your score for Autonomy is ${Math.round(subscales?.autonomy || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.autonomy || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]

Personal Growth (${Math.round(subscales?.personal_growth || 0)}/${dimensionMaxScore}): Your score for Personal Growth is ${Math.round(subscales?.personal_growth || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.personal_growth || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]

Purpose in Life (${Math.round(subscales?.purpose_in_life || 0)}/${dimensionMaxScore}): Your score for Purpose in Life is ${Math.round(subscales?.purpose_in_life || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.purpose_in_life || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]

Self Acceptance (${Math.round(subscales?.self_acceptance || 0)}/${dimensionMaxScore}): Your score for Self Acceptance is ${Math.round(subscales?.self_acceptance || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.self_acceptance || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]

Positive Relations (${Math.round(subscales?.positive_relations || 0)}/${dimensionMaxScore}): Your score for Positive Relations is ${Math.round(subscales?.positive_relations || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.positive_relations || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]

Environmental Mastery (${Math.round(subscales?.environmental_mastery || 0)}/${dimensionMaxScore}): Your score for Environmental Mastery is ${Math.round(subscales?.environmental_mastery || 0)} out of ${dimensionMaxScore}, which places you in the ${this.getScoreStatus(subscales?.environmental_mastery || 0, assessmentType)} range. [Write ONLY what this score means about their inner experience. NO advice. NO suggestions. NO "try" or "you could". Just describe their psychological state.]
