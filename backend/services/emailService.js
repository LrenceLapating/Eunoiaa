...
class EmailService {
   ...
   * @param {string} resetData.userType - User type (student/counselor)
   * @param {string} resetData.resetUrl - Password reset URL
   * @returns {Promise<Object>} Result of email sending
   */
  async sendPasswordResetEmail(resetData) {
    try {
      const { email, name, userType, resetUrl } = resetData;

      if (!email || !name || !userType || !resetUrl) {
        throw new Error('Missing required password reset data for email');
      }
   ...
   */
  async validatePasswordResetToken(accessToken) {
    try {
      if (!accessToken) {
